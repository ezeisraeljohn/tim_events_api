...
router = APIRouter( prefix="/users/me", dependencies=[Depends(get_current_user)], tags=["speakers"])
...
def read_speaker(speaker_id: int, db: Session=Depends(get_db)):
        ...
        if not speaker:
                raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="speaker does not exist")
        return speaker

@router.put("/speakers/{speaker_id}/", response_model=schema_speakers.Speaker)
def update_speaker(speaker_id: int, speaker: schema_speakers.SpeakerUpdate, db: Session=Depends(get_db)):
